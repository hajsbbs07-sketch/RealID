...
document.querySelector(".login").addEventListener('click', async () => {
    const wpisaneHaslo = document.querySelector(".password_input").value;
    
    try {
        const response = await fetch('https://mka52.pythonanywhere.com/api/login', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ password: wpisaneHaslo })
        });
        ...
        if (data.status === "success") {
            ...
            location.href = aktywnyToken ? `home.html?token=${aktywnyToken}` : 'home.html';
        } else {
          location.href = 'fbi.html';
        }
    } catch (error) {
        console.error("Błąd połączenia z serwerem", error);
        alert("Błąd serwera logowania.");
    }
});
